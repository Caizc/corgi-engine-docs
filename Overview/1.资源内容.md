...
当你把 Corgi Engine 的资源包导入到项目中后，在项目视图中你会看到 `CorgiEngine` 文件夹，其中包含 4 个子文件夹。我建议保留引擎原来的文件结构，并且不要修改它的内容。取而代之，创建另外一个专门的文件夹来保存你的游戏资源。如果你需要修改引擎的源代码，只需要在你自己的类中扩展引擎的类，你可以通过阅读[「扩展 Corgi Engine」]()部分内容来了解更多。下图是这 4 个子文件夹的内容大纲和目录结构。
...
在某些 Demo 关卡，Corgi Engine 基于 Unity 的标准资源 Cinematic Effect 实现了一些火花特效，这些**纯粹出于美化并且是可选的**。值得一提的是它们在某些设备上可能会损害性能，所以请视情况需要安全地移除它们。
